...
export default function TabelaMotoboys(props) {
    ...
    function filtraMotoboys(e) {
        const termoBusca = e.currentTarget.value;

        fetch(urlBase + "https://129.146.68.51/aluno45-pfsii/motoboy", { method: "GET" })
    ...
    return (
        <Container>
           <center><Button onClick={() => {
            ...
            <Container>
                <Row className="m-3">
                    <Form.Control type="text" placeholder="Busque um nome abaixo" id="termoBusca" onChange={filtraMotoboys} />
                </Row>
            </Container>
                ...
                <tbody>
                    {
                        props.listaMotoboys?.map((motoboy) => {
                            return <tr key={motoboy.ID}>
                                <td>{motoboy.ID}</td>
                                <td>{motoboy.nome}</td>
                                <td>{motoboy.cpf}</td>
                                <td>{motoboy.endereco}</td>
                                <td>{motoboy.bairro}</td>
                                <td>{motoboy.habilitacao}</td>
                                <td>{motoboy.fone}</td>
                                 <td>
                                    <Button onClick={() => { props.editarMotoboy(motoboy) }}>
