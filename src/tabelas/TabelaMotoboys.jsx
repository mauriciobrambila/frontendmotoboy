    ...
    fetch(urlBase3, { method: "GET" })
      ...
      .then((listaMotoboys) => {
        if (Array.isArray(listaMotoboys)) {
          const resultadoBusca = listaMotoboys.filter((motoboy) =>
          motoboy.nome.toLowerCase().includes(termoBusca.toLowerCase())
          );
          props.setMotoboys(resultadoBusca);
        }
      });
  }
      ...
      <Table striped bordered hover size="sm">
        <thead>
          <tr>
            ...
            <th><center>Data Nascimento</center></th>
          ...
          {props.listaMotoboys?.map((motoboy) => {
            return (
              <tr key={motoboy.cpf}>
                <td>{motoboy.nome}</td>
                <td>{motoboy.cpf}</td>
                <td>{motoboy.dataNasc}</td>
                <td>{motoboy.email}</td>
                <td>{motoboy.tel}</td>
                <td>{motoboy.sexo}</td>
                <td>{motoboy.cidade}</td>
                <td>{motoboy.uf}</td>
